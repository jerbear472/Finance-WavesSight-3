...
-- Only verifiers can create verifications
CREATE POLICY "Verifiers can create verifications" ON finance_trend_verifications
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE id = auth.uid() 
            AND (is_admin = true OR role = 'verifier')
        )
    );
